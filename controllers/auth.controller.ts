...
const validarToken = async (req: Request, res: Response) => {
    ...
    const authorizationBody = req.body.authorization;
    // if (!authorization) return res.status(401).json({ error: 'No token provided' })
    // if (!authorization.startsWith('Bearer ')) return res.status(401).json({ error: 'Invalid Bearer token' })

    if (!authorizationBody) return res.status(401).json({ error: 'No token provided' })
    if (!authorizationBody.startsWith('Bearer ')) return res.status(401).json({ error: 'Invalid Bearer token' })

    // const token: any = authorization.split(' ').at(1) || '';
    const token: any = authorizationBody.split(' ').at(1) || '';
